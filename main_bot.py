...
def start_message(message):
    bot.send_message(message.chat.id, "Приветсвую, похоже ты в первый раз пользуешься этим ботом. "
                                      "Давай знакомться, меня зовут FKIT_ALARM bot. Чтобы использовать бота мне надо "
                                      "узнать некоторую информацию от тебя, приступим!\nP.S. Внизу есть клавиатура, "
                                      "для ввода определенного поля нажми кнопку на клавиатуре и приступай к "
                                      "заполнению")
    info_marcup = telebot.types.ReplyKeyboardMarkup(resize_keyboard=True)
    info_marcup.add(telebot.types.KeyboardButton("Ввести ФИО"))
    info_marcup.add(telebot.types.KeyboardButton("Ввести номер группы"))
    info_marcup.add(telebot.types.KeyboardButton("Закончить ввод"))
    bot.send_message(message.chat.id, "Выберите пункт из меню клавиатуры:", reply_markup=info_marcup)
...
def make_GROUP_false():
    GROUP_in_process = False

@bot.message_handler(content_types=['text'])
def text_entering(message):
    if (FIO_in_process):
        FIO = message.text
        make_FIO_false()
    if (GROUP_in_process):
        try:
            ...
            bot.send_message(message.chat_id, "Кажется ты неверно ввел номер группы, попробуй ещё раз)")

@bot.message_handler(func=lambda message: message.text == "Ввести ФИО")
def FIO_entering(message):
    FIO_in_process = True
